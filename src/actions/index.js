...
export const createNewContact = ({ first_name, last_name, email, phone, company, project, notes }) => {
	const { currentUser } = firebase.auth();
	
	return (dispatch) => {
		firebase.database().ref(`/users/${currentUser.uid}/people`)
			.push({ first_name, last_name, email, phone, company, project, notes })
			.then(() => {
...
export const saveContact = ({ first_name, last_name, email, phone, company, project, notes, uid}) => {
	const { currentUser } = firebase.auth();
	
	return (dispatch) => {
		firebase.database().ref(`/users/${currentUser.uid}/people/${uid}`)
			.set({ first_name, last_name, email, phone, company, project, notes, uid})
			.then(() => {
